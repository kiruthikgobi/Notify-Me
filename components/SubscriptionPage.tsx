    ...
    <div className="space-y-10 animate-in fade-in duration-500 max-w-4xl mx-auto py-10 pb-20">
      <header className="text-center space-y-4">
        ...
        <p className="text-slate-500 dark:text-slate-400 font-medium text-lg">
          {isPro ? 'Your PRO workspace is active.' : 'Upgrade to Pro to add unlimited vehicles'}
        </p>
      </header>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div className="ui-card p-8 rounded-[2.5rem] border-l-8 border-primary-600 flex flex-col justify-between">
          ...
          <div className="mt-10 pt-8 border-t border-slate-100 dark:border-slate-800">
             <div className="flex items-end justify-between">
                <div>
                   <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Asset Registry Usage</p>
        ...
        <div className={`ui-card p-8 rounded-[2.5rem] relative overflow-hidden flex flex-col transition-all ${isPro ? 'opacity-50 grayscale' : 'shadow-2xl shadow-primary-500/10 border-primary-100'}`}>
          ...
          <div className="flex-1">
            <h3 className="text-2xl font-display font-bold mb-6">Pro Plan Features</h3>
            <div className="space-y-5">
              {[
                { label: 'Unlimited vehicle additions', icon: ICONS.Truck },
                { label: 'All expiry email alerts', icon: ICONS.Bell },
                { label: 'AI Audit Insights', icon: ICONS.Brain },
                { label: 'Automated Reminders', icon: ICONS.Mail },
                { label: 'Priority Support', icon: ICONS.Check }
              ].map((benefit, i) => (
          ...
          <div className="mt-10 pt-8 border-t border-slate-100 dark:border-slate-800 space-y-6">
            ...
            <button 
              disabled={isPro}
              onClick={onUpgrade}
              className={`w-full py-5 rounded-2xl font-black text-xs tracking-widest transition-all shadow-xl active:scale-95 ${isPro ? 'bg-slate-100 text-slate-400' : 'bg-primary-600 text-white hover:bg-primary-700 shadow-primary-500/20'}`}
            >
              {isPro ? 'PLAN ALREADY ACTIVE' : 'Subscribe to Pro – ₹99/year'}
            </button>
          </div>
        </div>
      </div>
